...
namespace EmployeeFunction
{
    public class Employee
    {
        ...
        public static IActionResult GetEmployees([HttpTrigger(AuthorizationLevel.Anonymous, "Get", Route = "GetEmployees")]HttpRequest req, ILogger log,
            [CosmosDB(databaseName: "Employee", collectionName: "Employee", ConnectionStringSetting = "CosmosDBConnection", SqlQuery = "Select * from c")]IEnumerable<EmployeeEntity> EmpEntity)
        {
            try
            {
        ...
        IAsyncCollector<object> todos, ILogger log)
        {
            try
            {
                ...
                var Emp = new EmployeeEntity() { employeeName = input.employeeName, employeeSalary = input.employeeSalary, location = input.location, address = input.address, pincode = input.pincode, date = input.date };
                ...
                await todos.AddAsync(new { id = Emp.employeeId, Emp.employeeName, Emp.employeeSalary, Emp.location, Emp.address, Emp.pincode, Emp.date});
